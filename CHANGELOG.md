...
### Added
- **DeepSeek Harness Adapter (Experimental)**: `adapters/deepseek-harness/{paths,detect}.{ps1,sh}` implementing the verified multi-root discovery model (`project-dsh` 100 / `project-agents` 200 / `custom` 300 / `user-dsh` 400 / `user-agents` 500 / `bundled` 600), within-layer rank duplicate resolution (`precedence=within-layer-rank`, `predicted_winner` with `winner_basis='filesystem-rank-root-order'`, `winner_confidence='within-layer-only'|'single-candidate'`, `runtime_winner=null`), DSH invocation policy parsing (`disable-model-invocation` / `user-invocable`, legacy-key rejection), the reserved `.system` namespace, and provider+consumer preset detection (`tool-skill` / `skill-search` / none). Catalog visibility is split into `expected_visible` (= `visible`) and `confirmed_visible` (null — another process's session is not observable), stamped with `visibility_confidence='inferred'` and `visibility_status='expected'`. Each variant carries a `platform_protected` string (`'false'` for user/project roots, `'unknown'` for bundled/custom whose mutability DSH has not defined) alongside skill-manager's own `write_policy`. The index includes a `dsh_duplicate_summary` block (physical entries, unique names, duplicate names/variants/candidate-variants, metadata conflicts, predicted/confirmed winners). Two-stage detection: `detected = cli OR package OR dsh_home OR config`; `skill_runtime_ready = provider AND consumer`; `usable = detected AND skill_runtime_ready` (with `usable_reason`).
- **DSH-aware Doctor & Fix**: doctor reports environment, provider/consumer state, watch state (no restart recommended unless `watch: false`), `catalog visibility: expected (inferred...)`, duplicates with a predicted within-layer winner (`basis: filesystem-rank-root-order`) and an explicit "does not claim a runtime winner" note, and the reserved namespace (`manager write policy: refuse`); fix touches only the single writable user-dsh variant and refuses bundled and custom (diagnostic-only) and DSH-rejected skills. `protected` stays `false` for every DSH variant; `platform_protected` and `write_policy` are separate dimensions.
- **DSH Install Targets**: `install -Agent deepseek-harness` with `-Scope user` (default `~/.dsh/skills`), `user-agents` (shared), and `project`; backups under `$DSH_HOME/skill-manager/backups`.
- **DeepSeek Harness Test Suite**: `tests/test-deepseek-harness.{ps1,sh}` covering two-stage detection (`~/.agents`-only != usable; bare `~/.dsh` without provider/consumer != usable; provider without consumer != runtime-ready; CLI+preset+consumer == usable), six-root duplicate resolution with `winner_basis`/`providers=['filesystem']`/`runtime_winner=null`, `.system` skip, flat `.md`, `user-invocable:false` (model visibility unaffected), legacy-key rejection, consumer gating, Find reuse, fix refusal (bundled/custom diagnostic-only), watch diagnostics, `platform_protected` per root, and `dsh_duplicate_summary` reconciliation.
- **CI Integration**: deepseek-harness test steps added to the Bash (Ubuntu/macOS) and PowerShell (Windows) matrix jobs.

### Changed
- Supported agent list now includes `deepseek-harness` (**Experimental**, not yet Supported) alongside `claude`, `codex`, `antigravity`.
- DeepSeek Harness remains Experimental until the platform's pre-release version contract stabilizes.
